...
if model_type == "Objek Deteksi":
    start = time.time()
    model_info = load_model("configs/yolov4-custom.cfg", "weights/yolov4-custom_best.weights", "obj.names")
    end = time.time()
...
st.sidebar.text("{:.4f} detik".format(end - start))
...
if img_stream:
    ...
    st.success("Waktu Deteksi: {:.4f} detik".format(end - start))
